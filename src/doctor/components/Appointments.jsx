...
const Appointments = () => {
  ...
  return (
    <div>
        <div className='lg:mx-2 grid grid-cols-6 gap-15 xs:w-75 xsm:w-75 mbl:w-95  md:w-160 sm:w-full bg-gray-100 p-3 rounded-t-lg shadow font-semibold text-gray-700'>
           <h1>Doctor</h1>
           <h1>Date</h1>
      ...
      <div className='flex flex-col gap-3 lg:mx-2'>
       {appointments?.data?.map(a=>(
        <div key={a.id} className='grid grid-cols-6 gap-15 xs:w-75 xsm:w-75 mbl:w-95  md:w-160 sm:w-fullp-3 items-center shadow-sm hover:bg-gray-50 transition'>
         <h1 className='text-sm tracking-tighter'>{a.doctor}</h1>
         <h1 className='tracking-tighter text-sm'>{a.date}</h1>
